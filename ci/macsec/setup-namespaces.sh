...
#   ns-a (10.0.0.1/veth) <--veth--> ns-b (10.0.0.2/veth)  [arnika UDP]
#   ns-a (172.16.0.1/macsec0) --- MACsec --- ns-b (172.16.0.2/macsec0)
#
set -e

# gcm-aes-256 or gcm-aes-xpn-256; Arnika installs 256-bit keys.
CIPHER="${CIPHER:-gcm-aes-256}"
...
# --- veth pair: ns-a <-> ns-b (underlay for arnika UDP + MACsec) ---
...
ip netns exec ns-a ip link add link veth-a-b macsec0 type macsec \
    sci "${SCI_A}" cipher "${CIPHER}" encrypt on
...
ip netns exec ns-b ip link add link veth-b-a macsec0 type macsec \
    sci "${SCI_B}" cipher "${CIPHER}" encrypt on
